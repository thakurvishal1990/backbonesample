...
(function($, window, document) {
	...
	$.fn.dataTableExt.oPagination.listbox1 = {
	/*
	...
	"fnInit": function (oSettings, nPaging, fnCallbackDraw) {
		...
		nPage.innerHTML = "Display: ";
		nPaging.appendChild(nPage);
		nPaging.appendChild(nInput);
		nPaging.appendChild(nOf);
		
		$(nInput).addClass("paginationSelect");
	...
	"fnUpdate": function (oSettings, fnCallbackDraw) {
		...
		for (var i = 0, iLen = an.length; i < iLen; i++) {
			...
			if(elSel.options.length != iPages) {
				...
				for (var j = 0; j < iPages; j++) { //add the pages
					...
					oOption.text = ((j) * oSettings._iDisplayLength+1)+"  -  "+Math.min(((j+1) * oSettings._iDisplayLength),oSettings.fnRecordsDisplay());
				...
				spans[1].innerHTML = "&nbsp;&nbsp;of&nbsp;&nbsp;" + oSettings.fnRecordsDisplay();
			}
		  elSel.value = iCurrentPage;
		}
		
		/*End of pagination list update */
		...
		//for ( var i=0, iLen=an.length ; i<iLen ; i++ )
		{
			...
			//var buttons = an[i].getElementsByTagName('span');
				$(an).find(".first").removeClass().addClass("first");
				$(an).find(".previous").removeClass().addClass("previous");
				$(an).find(".next").removeClass().addClass("next");
				$(an).find(".last").removeClass().addClass("last");

			if ( oSettings._iDisplayStart === 0 )
			{
					$(an).find(".first").addClass("paginate_disabled_first");
					$(an).find(".previous").addClass("paginate_disabled_previous");
			}
			else
			{
			
				$(an).find(".first").addClass("paginate_enabled_first");
				$(an).find(".previous").addClass("paginate_enabled_previous");

			}
			
			if ( oSettings.fnDisplayEnd() == oSettings.fnRecordsDisplay() )
			{
				$(an).find(".next").addClass("paginate_disabled_next");
				$(an).find(".last").addClass("paginate_disabled_last");
			}
			else
			{
				$(an).find(".next").addClass("paginate_enabled_next");
				$(an).find(".last").addClass("paginate_enabled_last");
			}
		}
